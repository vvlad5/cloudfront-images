define(["exports"], (function (t) {
    ...
    function h(t, e, n) {
        ...
        } else if (t instanceof RegExp) a = new r(t, e, n); else if ("function" == typeof t) a = new i(t, e, n); else {
    ...
    const u = {
      ...
      }, l = t => [u.prefix, t, u.suffix].filter((t => t && t.length > 0)).join("-"), f = t => t || l(u.precache),
      w = t => t || l(u.runtime);
    ...
    let m, R;
    const v = new WeakMap, b = new WeakMap, q = new WeakMap, D = new WeakMap, U = new WeakMap;
    let x = {
        get(t, e, s) {
    ...
    function L(t) {
        return t !== IDBDatabase.prototype.transaction || "objectStoreNames" in IDBTransaction.prototype ? (R || (R = [IDBCursor.prototype.advance, IDBCursor.prototype.continue, IDBCursor.prototype.continuePrimaryKey])).includes(t) ? function (...e) {
            return t.apply(C(this), e), E(v.get(this))
        } : function (...e) {
            return E(t.apply(C(this), e))
        } : function (e, ...s) {
            const n = t.call(C(this), e, ...s);
            return q.set(n, e.sort ? e.sort() : [e]), E(n)
        }
    }
    ...
    function I(t) {
        return "function" == typeof t ? L(t) : (t instanceof IDBTransaction && function (t) {
            ...
            b.set(t, e)
        }(t), g(t, m || (m = [IDBDatabase, IDBObjectStore, IDBIndex, IDBCursor, IDBTransaction])) ? new Proxy(t, x) : t)
    }
    ...
    const C = t => U.get(t);
    const N = ["get", "getKey", "getAll", "getAllKeys", "count"], O = ["put", "add", "delete", "clear"], B = new Map;

    function k(t, e) {
        if (!(t instanceof IDBDatabase) || e in t || "string" != typeof e) return;
        if (B.get(e)) return B.get(e);
        const s = e.replace(/FromIndex$/, ""), n = e !== s, i = O.includes(s);
        if (!(s in (n ? IDBIndex : IDBObjectStore).prototype) || !i && !N.includes(s)) return;
    ...
    x = (t => y({}, t, {get: (e, s, n) => k(e, s) || t.get(e, s, n), has: (e, s) => !!k(e, s) || t.has(e, s)}))(x);
    try {
        self["workbox:expiration:6.6.0"] && _()
    } catch (t) {
    }
    const M = "cache-entries", T = t => {
    ...
    class W {
        ...
        p(t) {
            this.l(t), this.u && function (t, {blocked: e} = {}) {
                const s = indexedDB.deleteDatabase(t);
                e && s.addEventListener("blocked", (t => e(t.oldVersion, t))), E(s).then((() => {
                }))
            }(this.u)
        }
        ...
        async setTimestamp(t, e) {
            const s = {url: t = T(t), timestamp: e, cacheName: this.u, id: this.m(t)},
              n = (await this.getDb()).transaction(M, "readwrite", {durability: "relaxed"});
            await n.store.put(s), await n.done
        }

        async getTimestamp(t) {
            const e = await this.getDb(), s = await e.get(M, this.m(t));
            return null == s ? void 0 : s.timestamp
        }

        async expireEntries(t, e) {
            const s = await this.getDb();
            let n = await s.transaction(M).store.index("timestamp").openCursor(null, "prev");
            const i = [];
            let r = 0;
            for (; n;) {
                const s = n.value;
        ...
        async getDb() {
            return this.h || (this.h = await function (t, e, {
                ...
                const a = indexedDB.open(t, e), o = E(a);
                return n && a.addEventListener("upgradeneeded", (t => {
                    n(E(a.result), t.oldVersion, t.newVersion, E(a.transaction), t)
            ...
            }("workbox-expiration", 1, {upgrade: this.p.bind(this)})), this.h
        }
    }

    class j {
        constructor(t, e = {}) {
            this.R = !1, this.v = !1, this.q = e.maxEntries, this.D = e.maxAgeSeconds, this.U = e.matchOptions, this.u = t, this.L = new W(t)
        }

        async expireEntries() {
            if (this.R) return void (this.v = !0);
            this.R = !0;
            const t = this.D ? Date.now() - 1e3 * this.D : 0, e = await this.L.expireEntries(t, this.q),
        ...
        async isURLExpired(t) {
            if (this.D) {
                const e = await this.L.getTimestamp(t), s = Date.now() - 1e3 * this.D;
                return void 0 === e || e < s
            }
            return !1
        }
    ...
    const P = {cacheWillUpdate: async ({response: t}) => 200 === t.status || 0 === t.status ? t : null};
    ...
    class F {
        constructor(t, e) {
            this._ = {}, Object.assign(this, e), this.event = e.event, this.I = t, this.C = new K, this.N = [], this.O = [...t.plugins], this.B = new Map;
        ...
        async cacheMatch(t) {
            const e = A(t);
            let s;
            const {cacheName: n, matchOptions: i} = this.I, r = await this.getCacheKey(e, "read"),
        ...
        async cachePut(t, e) {
            ...
            if (!e) throw new s("cache-put-with-no-response", {url: (a = r.url, new URL(String(a), location.href).href.replace(new RegExp(`^${location.origin}`), ""))});
            var a;
            const o = await this.k(e);
            if (!o) return !1;
            const {cacheName: c, matchOptions: h} = this.I, u = await self.caches.open(c),
              l = this.hasCallback("cacheDidUpdate"), f = l ? await async function (t, e, s, n) {
                  const i = S(e.url, s);
                  if (e.url === i) return t.match(e, n);
                  const r = Object.assign(Object.assign({}, n), {ignoreSearch: !0}), a = await t.keys(e, r);
        ...
        async getCacheKey(t, e) {
            ...
            if (!this._[s]) {
                let n = t;
                for (const t of this.iterateCallbacks("cacheKeyWillBeUsed")) n = A(await t({
                    mode: e,
                    request: n,
                    event: this.event,
                    params: this.params
        ...
        * iterateCallbacks(t) {
            for (const e of this.I.plugins) if ("function" == typeof e[t]) {
                const s = this.B.get(e), n = n => {
    ...
    class H {
        ...
        handleAll(t) {
            ...
            const e = t.event, s = "string" == typeof t.request ? new Request(t.request) : t.request,
              n = "params" in t ? t.params : void 0, i = new F(this, {event: e, request: s, params: n}),
              r = this.M(i, s, e);
        ...
        async M(t, e, n) {
            ...
            try {
                if (i = await this.W(e, t), !i || "error" === i.type) throw new s("no-response", {url: e.url})
            } catch (s) {
    ...
    class J {
        constructor({precacheController: t}) {
            this.cacheKeyWillBeUsed = async ({request: t, params: e}) => {
                const s = (null == e ? void 0 : e.cacheKey) || this.j.getCacheKeyForURL(t.url);
    ...
    class Y extends H {
        constructor(t = {}) {
            t.cacheName = f(t.cacheName), super(t), this.P = !1 !== t.fallbackToNetwork, this.plugins.push(Y.copyRedirectedCacheableResponsesPlugin)
        }
        ...
        async K(t, e) {
            let n;
            const i = e.params || {};
            if (!this.P) throw new s("missing-precache-entry", {cacheName: this.cacheName, url: t.url});
            {
                const s = i.integrity, r = t.integrity, a = !r || r === s;
                n = await e.fetch(new Request(t, {integrity: "no-cors" !== t.mode ? r || s : void 0})), s && a && "no-cors" !== t.mode && (this.A(), await e.cachePut(t, n.clone()))
            }
            return n
        }
    ...
    class Z {
        constructor({cacheName: t, plugins: e = [], fallbackToNetwork: s = !0} = {}) {
            this.F = new Map, this.H = new Map, this.$ = new Map, this.I = new Y({
                cacheName: f(t),
        ...
        addToCacheList(t) {
            ...
            for (const n of t) {
                ...
                const {cacheKey: t, url: i} = G(n), r = "string" != typeof n && n.revision ? "reload" : "default";
                if (this.F.has(i) && this.F.get(i) !== t) throw new s("add-to-cache-list-conflicting-entries", {
                    firstEntry: this.F.get(i),
                    secondEntry: t
                });
                if ("string" != typeof n && n.integrity) {
                    if (this.$.has(t) && this.$.get(t) !== n.integrity) throw new s("add-to-cache-list-conflicting-integrities", {url: i});
        ...
        install(t) {
            return $(t, (async () => {
                ...
                for (const [e, s] of this.F) {
                    const n = this.$.get(s), i = this.H.get(e),
    ...
    t.ExpirationPlugin = class {
        constructor(t = {}) {
            ...
            }, this.X = t, this.D = t.maxAgeSeconds, this.Y = new Map, t.purgeOnQuotaError && function (t) {
                p.add(t)
        ...
        constructor(t = {}) {
            super(t), this.plugins.some((t => "cacheWillUpdate" in t)) || this.plugins.unshift(P)
        }
